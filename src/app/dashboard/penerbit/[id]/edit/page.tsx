import Link from "next/link";
import { notFound } from "next/navigation";
import { db } from "@/lib/db";
import { EntityForm } from "@/components/entity-form";
import { updatePenerbitAction } from "../../actions";

interface PageProps {
  params: Promise<{ id: string }>;
}

export default async function PenerbitEditPage({ params }: PageProps) {
  const { id } = await params;
  const item = await db.penerbit.findUnique({ where: { id_penerbit: Number(id) } });
  if (!item) notFound();

  return (
    <div className="max-w-md">
      <Link href="/dashboard/penerbit" className="text-sm text-slate-500 hover:text-slate-900">
        ← Kembali
      </Link>
      <h1 className="mt-2 text-lg font-semibold text-slate-900">Edit Penerbit</h1>

      <div className="mt-4 rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <EntityForm
          action={updatePenerbitAction.bind(null, item.id_penerbit)}
          fields={[
            { type: "text", name: "nama_penerbit", label: "Nama Penerbit", required: true },
            { type: "textarea", name: "alamat", label: "Alamat" },
            { type: "text", name: "telepon", label: "Telepon" },
          ]}
          defaultValues={{
            nama_penerbit: item.nama_penerbit,
            alamat: item.alamat ?? "",
            telepon: item.telepon ?? "",
          }}
          submitLabel="Simpan Perubahan"
        />
      </div>
    </div>
  );
}
